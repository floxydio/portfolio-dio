...
(async () => {
	...
	if (dataJson.githubUsername !== 'floxydio') {
	...
	// Open data.json, merge it with data for floxydio and save it to disk.
	const dataPath = path.join(process.cwd(), 'data.json');
	const newData = { ...dataJson, ...data };
	// Write it back to disk.
	await fs.writeFile(dataPath, JSON.stringify(newData, null, 4));

	console.log('⚙️  Reverted to template data (using floxydio).');
})();
